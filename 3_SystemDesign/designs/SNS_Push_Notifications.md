...
### 2.SNS Topic Object [ARN] (API key used when creating the topic):

- In the AWS Console or using the AWS SDK, you create an SNS Platform Application for both iOS (APNS) and Android (FCM).
...
```
### 2. Create Platform Endpoint (AWS SNS):
...
```
### 3. Save Endpoint ARN to Database:
