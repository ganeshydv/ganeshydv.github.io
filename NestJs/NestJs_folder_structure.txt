...
=============================================================================
      ...
      2] [Connection() = createConnection( {config obj} )]. getConnection()
          : TYPEORM
          - perform CRUD using instance of Connection (verified)

      3] DataSource directly

 -Accesing using 1) Repository 2) EntityManager 3) DataSource 4) Connection
      5) queryBuilder
-------------------------------------------------------------------------
...
7) each service can be enjected independently in each module but it will not be
  singleton means for each module it will be new instance
- SOLUTION : if you want to use the same object of service for all other 
